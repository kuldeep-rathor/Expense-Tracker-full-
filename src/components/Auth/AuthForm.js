...
const AuthForm = () => {
  ...
  const [isLoading, setIsLoading] = useState(false); //its used for loading where submitting the request
  ...
  const submitHandler = (event) => {
    ...
    if (isLogin) {
      url =
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBAszjqqCksC6UJ-LHEMZExLpQmtFHBCvY";
    } else {
      url =
        "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBAszjqqCksC6UJ-LHEMZExLpQmtFHBCvY";
    }
