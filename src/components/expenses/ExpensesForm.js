...
const ExpensesForm = () => {
  ...
  //i have created this function to download csv file when called i click on that it will use csvExportSlice
  ...
  const firebaseUrl =
    "https://expense-tracker-bc21f-default-rtdb.firebaseio.com";
  ...
  const submitHandler = async (event) => {
    ...
    //we will send amount,description,category and email to server
    const expenseData = {
      amount: amount,
    ...
    try {
      const response = await axios.post(
        ...
        expenseData, //while sending req in url at last append email which i will enter dynamically which in later i can use to fetch the data for a particular user
        {
          headers: {
  ...
  const editExpenseHandler = async (expenseId) => {
    const updatedExpense = expenseList.find(
          ...
          <Form.Group>
            <Dropdown>
              ...
              <Dropdown.Menu>
                ...
                <Dropdown.Item onClick={() => handleCategorySelect("School")}>
                  School
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
